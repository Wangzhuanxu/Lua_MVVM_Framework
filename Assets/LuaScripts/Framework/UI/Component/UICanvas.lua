...
-- 创建
function UICanvas:OnCreate(item,binder,relative_order)
	base.OnCreate(self)
	-- Unity侧原生组件
	...
	local layer = self.view.holder.info
	self.unity_canvas.sortingLayerID = layer.sorting_layer_id
...
-- 获取层级内order
function UICanvas:GetOrder()
	return self.relative_order
end

-- 设置层级内order
function UICanvas:SetOrder(relative_order)
